...
//	TableView<Member> tableView2;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		btnAdd.setOnAction(new EventHandler<ActionEvent>() {
			...
			@Override
			public void handle(ActionEvent arg0) {
				//ObservableList<Book> list=getStudentList();
				handleBtnAddAction();
			}	
		});
	}// end of initialize()
	
	public ObservableList<Book> getStudentList(){
		Connection conn = ConnectionDB.getDB();
		String sql = "select * from BOOKDB1";
		list = FXCollections.observableArrayList();
		...
		try {
			...
			while(rs.next()) {
				Book book= new Book(rs.getString("bookname"),rs.getString("bookauthor"),rs.getString("bookpublisher"),rs.getInt("bookprice"));
				list.add(book);
			}
